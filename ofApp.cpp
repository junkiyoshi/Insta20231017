...
void ofApp::update() {
	...
	if (param > 12 && param < 38) {
		...
		if (param < 25) {
			...
			this->noise_step += ofMap(param, 12, 25, 0, 0.1);
		}
		else if (param > 25) {

			this->noise_step += ofMap(param, 25, 38, 0.1, 0);
		}
	}
	...
	for (int i = 0; i < 10; i++) {
		...
		auto noise_value = ofNoise(ofRandom(1000), this->noise_step);
		height_noise_value_list.push_back(noise_value);
		total_height_noise_value += noise_value;

		noise_value = ofNoise(ofRandom(1000), this->noise_step);
	...
	for (int i = 0; i < 10; i++) {
		...
		this->setBoxToMesh(this->face, this->frame, glm::vec3(0, y, 0), height - 4, len, len, ofMap(ofNoise(ofRandom(360), this->noise_step), 0, 1, -360, 360));
		prev_height = height;
	}
}
